...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[Try[URL]] =
      for (n <- ns \\ "a") yield createURL(Option(u), n \@ "href")
    // TO BE IMPLEMENTED
    ...
    // 9 points.
    for {s <- getURLContent(u); c <- MonadOps.asFuture(getLinks(s))} yield c // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    Future.sequence(for(x <- for(u <- us2) yield wget(u)) yield MonadOps.sequence(x))
